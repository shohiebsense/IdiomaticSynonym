...
*/
class UnderliningServiceUsingTokenization constructor (val context: Context) {
 ...
 */
    ...
    fun init() : UnderliningServiceUsingTokenization {
    ...
    fun convertToCharSequence(list : ArrayList<CombinedIdiom>): ArrayList<Link> {
        ...
        MaxentTagger.tokenizeText(StringReader(extractedPdfTexts.toString())).forEachIndexed { sentenceIndex, sentenceChar ->
            ...
            for(combinedIdiom in list){
                var index = sentence.toLowerCase().indexOf(combinedIdiom.idiom.toLowerCase(),0,true)
                //  val matcher =Pattern.compile( combinedIdiom.idiom+"\\b",Pattern.CASE_INSENSITIVE).matcher(sentence)
                // if(matcher.find()){
                if(index >= 0 && (!sentence.get(index-1).isLetter() || !sentence.get(index+combinedIdiom.idiom.length+1).isLetter()) && singleCombinedIdiom.add(combinedIdiom.idiom)) {
                    AppUtil.makeDebugLog("founded")
                    indexedSentences.add(Link(combinedIdiom.idiom))
                    ...
                    var lastIndex = index+combinedIdiom.idiom.length

                    spannableStringBuilder.setSpan(StyleSpan(Typeface.BOLD), index, lastIndex, Spannable.SPAN_EXCLUSIVE_INCLUSIVE)
                    ...
                    spannableStringBuilder.setSpan(URLSpan(combinedIdiom.idiom),index,lastIndex,Spanned.SPAN_EXCLUSIVE_INCLUSIVE)
        ...
        timer.stop()
        Log.e("shohiebsense ","time ellapsedd "+timer.time)
    ...
    fun getError(e: Unit){
        ...
        underliningCallback.onErrorTranslatingText(e.toString())
    }
